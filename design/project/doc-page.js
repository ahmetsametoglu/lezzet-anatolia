...
(() => {
  ...
  class DocPage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Slotted header/footer can gain layout after this element connects
      // (streamed templates, late webfonts): keep re-measuring until the
      // spacers match the real slot heights.
      [0, 120, 400, 1200].forEach((t) => setTimeout(() => this._scheduleMeasure(), t));
    }
    ...
    _measure() {
      ...
      if (typeof ResizeObserver === 'function') {
        if (!this._ro) this._ro = new ResizeObserver(() => this._scheduleMeasure());
        if (this._roTargets !== (hdr ? 1 : 0) + (ftr ? 2 : 0)) {
          this._ro.disconnect();
          if (hdr) this._ro.observe(hdr);
          if (ftr) this._ro.observe(ftr);
          this._roTargets = (hdr ? 1 : 0) + (ftr ? 2 : 0);
        }
      }
