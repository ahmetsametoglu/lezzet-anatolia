...
- **Depo seçim kartları (açılış hâli)** — kapsamı çok depolu personel sayfaya girdiğinde, boş bir "önce depo seçin" duvarı yerine **kart ızgarası** görür: her tesis için ad · kod ve bekleyen işin kırılımı (bugün · kargo · geciken). Karta tıklamak o depoyu seçer ve kuyruğu açar. **Varsayılan depo yoktur** kuralı bozulmuyor — hiçbir kart önceden seçili ya da "önerilen" değil; değişen tek şey, seçimin bir engel değil sayfanın ilk adımı olarak kurulması ve **bilgiyle beslenmesi**: operatör hangi depoda iş olduğunu görerek seçer, adını hatırlayarak değil. Geciken sayısı vurgulu, sıfırsa hiç çizilmez (her kartta duran bir "0 geciken", gerçek gecikmeyi görünmez yapar)
