...
- **Para hareketleri listesi** — her satır: hesap, yön (giriş/çıkış), tutar, tip (sipariş ödemesi / iade / stok alımı / gider / transfer / sermaye / sair), tür (kira, akaryakıt, maaş, reklam… — tek), cari (kime ödendi / kimden geldi), serbest etiketler, tarih, açıklama; bağlıysa sipariş, alım ya da belge referansı. Süzgeç: hesap (bakiye kartının kendisi), tip, tarih aralığı. İşi satırın kendisinde (ayrı panel yok): neyle izah edildiği ("Karşılığı" — bağ, öneri, onay, geri alma), sınıflandırması (tür · cari · etiket). Satırda bilginin önemi: tutar en belirgin, sonra gün (liste günlere göre gruplu), sonra açıklama
- **Belgeler** — fatura, fiş, dekont: tutarı, KDV'si, **açık kalanı** (ödenmemiş fatura bir borçtur) ve hangi hareketlerle ödendiği. Hareketlerin yanında ayrı bir liste; "yalnız açık" süzgeci ödenmeyi bekleyenleri gösterir. Satırda belge tutarı en belirgin, altında açık kalanı; liste aylara göre gruplu
...
- **Transfer** — hesaplar arası taşıma (nakit→banka, Stripe→banka aktarımı): tek işlem, iki hesapta simetrik hareket; kullanıcı "gelir/gider" diye düşünmek zorunda kalmaz; elle hareketle aynı pencerede, kiplerinden biri — alanlar her kipte aynı yerde
