...
beforeAll(async () => {
  const category = await new CategoryService(db).create({ name: { tr: `Sepet testi ${damga}` } });
  ...
  const profile = await profiles.insert({ name: `Sepet müşterisi ${damga}` });
...
afterAll(async () => {
  await purgeTestData(db, { productIds: [productId], categoryIds: [categoryId], profileIds: acilanlar });
});
...
describe('sepet durumu (07.1)', () => {
  ...
  it('kalem eklenir ve sunucuda kalır (cihaz değişse de durur)', async () => {
    ...
    const baskaCihaz = await new CartService(db).get(customerId);
    expect(baskaCihaz.items).toEqual([expect.objectContaining({ variantId: variantA, qty: 2, unitPrice: 12.5 })]);
  });

  it('aynı satır tekrar eklenince ADET birleşir, ikinci satır açılmaz', async () => {
    await carts.addItem(customerId, { variantId: variantA, qty: 2, unitPrice: 12.5 });
    const sepet = await carts.addItem(customerId, { variantId: variantA, qty: 3, unitPrice: 14 });

    expect(sepet.items).toHaveLength(1);
    expect(sepet.items[0]).toMatchObject({ qty: 5, unitPrice: 12.5 }); // gösterilen fiyat ilkinde kalır
  });

  it('farklı varyantlar ayrı satırdır', async () => {
    await carts.addItem(customerId, { variantId: variantA, qty: 1, unitPrice: 12.5 });
    const sepet = await carts.addItem(customerId, { variantId: variantB, qty: 1, unitPrice: 22 });
    expect(sepet.items).toHaveLength(2);
  });

  it('teklif satırı (partiye çıpalı) normal satırdan AYRI yaşar', async () => {
    const stockId = crypto.randomUUID();
    await carts.addItem(customerId, { variantId: variantA, qty: 1, unitPrice: 12.5 });
    const sepet = await carts.addItem(customerId, { variantId: variantA, qty: 1, unitPrice: 8, stockId });

    expect(sepet.items).toHaveLength(2); // aynı ürün, biri indirimli parti — müşteri ikisini de ister
    expect(sepet.items.find((i) => i.stockId === stockId)?.unitPrice).toBe(8);
  });
});
...
describe('adet ve çıkarma', () => {
  it('adet belirlenir', async () => {
    await carts.addItem(customerId, { variantId: variantA, qty: 2, unitPrice: 12.5 });
    const sepet = await carts.setQty(customerId, variantA, 7);
    expect(sepet.items[0]!.qty).toBe(7);
  });
  ...
  it('çıkarma yalnız o satırı düşürür — teklif satırı ayrı kalır', async () => {
    ...
    const sepet = await carts.removeItem(customerId, variantA);
    expect(sepet.items).toHaveLength(1);
    expect(sepet.items[0]!.stockId).toBe(stockId);
  });
...
describe('anonim sepeti devralma (07.1)', () => {
  it('giriş yapınca sunucudaki sepet KORUNUR, gelen kalemler üstüne eklenir', async () => {
    ...
    const sepet = await carts.takeOver(customerId, [
      { variantId: variantA, qty: 1, unitPrice: 13 }, // çakışan satır → adet toplanır
      { variantId: variantB, qty: 4, unitPrice: 22 }, // yeni satır
    ]);

    expect(sepet.items).toHaveLength(2);
    expect(sepet.items.find((i) => i.variantId === variantA)).toMatchObject({ qty: 3, unitPrice: 12.5 });
    expect(sepet.items.find((i) => i.variantId === variantB)?.qty).toBe(4);
  });

  it('sunucuda sepet yoksa gelen kalemler olduğu gibi devralınır', async () => {
    const sepet = await carts.takeOver(customerId, [{ variantId: variantB, qty: 1, unitPrice: 22 }]);
    expect(sepet.items).toHaveLength(1);
  });
