...
// Parti, rezervasyon ve kullanılabilir hesabı artık DEPO İÇİNDE (DOMAIN §17) — testin kendi
// deposu olmadan tek satır bile yazılamaz. Damgalı kod: üç ajan tek yerel DB'yi paylaşıyor.
let warehouseId: string;
const orderId = () => crypto.randomUUID();

beforeAll(async () => {
  warehouseId = (await createTestWarehouse(db, { label: 'STOK' })).id;
...
describe('kullanılabilir stok — türetilir, saklanmaz (06.2)', () => {
  ...
  it('kullanılabilir = fiili − aktif rezervasyon', async () => {
    ...
    await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 3 });

    expect(await stocks.getAvailable(warehouseId, variantId)).toMatchObject({ physicalQty: 10, reservedQty: 3, availableQty: 7 });
  });

  it('süresi dolmuş rezervasyon sayılmaz — görünüm süpürücüyü beklemez', async () => {
    await stocks.insert({ variantId, warehouseId, physicalQty: 5, expiryDate: dayOffset(200) });
  ...
  it('tarihi geçmiş DLC partisi OLGU olarak ayrı sayılır (satma kararı motorun)', async () => {
    ...
    await stocks.insert({ variantId, warehouseId, physicalQty: 6, expiryDate: dayOffset(-1) });
  ...
  it('çok varyantlı okuma tek turda ve eksiksiz döner', async () => {
    ...
    const map = await stocks.getAvailableMap(warehouseId, [variantId, unknown]);
    expect(map.size).toBe(2);
    expect(map.get(unknown)?.availableQty).toBe(0);
  });

  it('partiler FEFO sırasında gelir; ürünün tarih alanları aynı turda', async () => {
    await stocks.insert({ variantId, warehouseId, physicalQty: 3, expiryDate: dayOffset(300) });
    await stocks.insert({ variantId, warehouseId, physicalQty: 2, expiryDate: dayOffset(100) });

    const batches = await stocks.listByVariantWithDates(warehouseId, variantId);
...
describe('atomik ayırma (06.3)', () => {
  it('kullanılabilir yeterse ayırır ve kalanı bildirir', async () => {
    await stocks.insert({ variantId, warehouseId, physicalQty: 8, expiryDate: dayOffset(200) });

    const result = await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 5 });
  ...
  it('yetmezse HİÇ yazmaz — kısmi ayırma yok', async () => {
    await stocks.insert({ variantId, warehouseId, physicalQty: 2, expiryDate: dayOffset(200) });

    const result = await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 3 });
    expect(result).toMatchObject({ ok: false, reservationId: null, available: 2 });
    expect((await stocks.getAvailable(warehouseId, variantId)).reservedQty).toBe(0);
  });

  it('YARIŞ: iki istek aynı anda son birimi isterse yalnız biri kazanır', async () => {
    await stocks.insert({ variantId, warehouseId, physicalQty: 1, expiryDate: dayOffset(200) });

    const [a, b] = await Promise.all([
      reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 1 }),
      reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 1 }),
    ]);

    expect([a.ok, b.ok].filter(Boolean)).toHaveLength(1); // aşırı-satış yok
    expect((await stocks.getAvailable(warehouseId, variantId)).availableQty).toBe(0);
  });
  ...
  it('YARIŞ: on eşzamanlı istek, beş birimlik stoktan yalnız beşini kapar', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 10 }, () => reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 1 })),
    );
  ...
  it('TTL verilirse rezervasyon süreli, verilmezse süresiz (kapıda/vadeli)', async () => {
    ...
    await reservations.reserve({ orderId: order, variantId, warehouseId, qty: 1, ttlMinutes: 30 });
    await reservations.reserve({ orderId: order, variantId, warehouseId, qty: 1 });
  ...
  it('partiye çıpalı ayırma yalnız O partinin kullanılabilirine bakar', async () => {
    ...
    const surplus = await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 3, stockId: offerBatch.id });
    expect(surplus).toMatchObject({ ok: false, available: 2 });

    const exact = await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 2, stockId: offerBatch.id });
    expect(exact.ok).toBe(true);
  });

  it('sıfır/negatif miktar reddedilir', async () => {
    await expect(reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 0 })).rejects.toThrow();
  });
});

describe('serbest bırakma ve TTL süpürme (06.4)', () => {
  it('sipariş kapanınca rezervasyonları düşer', async () => {
    await stocks.insert({ variantId, warehouseId, physicalQty: 6, expiryDate: dayOffset(200) });
    const order = orderId();
    await reservations.reserve({ orderId: order, variantId, warehouseId, qty: 4 });

    await reservations.releaseByOrder(order);
    expect((await stocks.getAvailable(warehouseId, variantId)).availableQty).toBe(6);
  });
  ...
  it('süpürücü yalnız süresi geçmişi siler; ikinci tarama no-op (idempotent)', async () => {
    ...
    await db.from('reservation').insert({
      order_id: expired,
      variant_id: variantId,
      // Ham insert de depo taşımak ZORUNDA (not null): RPC'yi atlayan yol da kuralı atlayamaz.
      warehouse_id: warehouseId,
      qty: 3,
      expires_at: new Date(Date.now() - 60_000).toISOString(),
    });
    await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 2, ttlMinutes: 30 }); // süresi dolmamış
    await reservations.reserve({ orderId: orderId(), variantId, warehouseId, qty: 1 }); // süresiz
    ...
    expect((await stocks.getAvailable(warehouseId, variantId)).reservedQty).toBe(3); // 2 süreli + 1 süresiz

    await reservations.sweepExpired();
    expect(await rowCount()).toBe(2); // yalnız süresi geçen gitti
    expect((await stocks.getAvailable(warehouseId, variantId)).reservedQty).toBe(3);
...
describe('eşik altı varyantlar (06.11 girdisi)', () => {
  it('kullanılabilir eşiğin altına düşen varyant listeye girer, üstündeki girmez', async () => {
    ...
    const below = await stocks.listBelowMinStock(warehouseId);
