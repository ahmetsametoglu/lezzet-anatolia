...
describe('kullanılabilir stok — türetilir, saklanmaz (06.2)', () => {
  ...
  it('kullanılabilir = fiili − aktif rezervasyon', async () => {
    await stocks.insert({ variantId, physicalQty: 10, expiryDate: gun(200) });
  ...
  it('süresi dolmuş rezervasyon sayılmaz — görünüm süpürücüyü beklemez', async () => {
    await stocks.insert({ variantId, physicalQty: 5, expiryDate: gun(200) });
  ...
  it('tarihi geçmiş DLC partisi OLGU olarak ayrı sayılır (satma kararı motorun)', async () => {
    await stocks.insert({ variantId, physicalQty: 4, expiryDate: gun(200) });
    await stocks.insert({ variantId, physicalQty: 6, expiryDate: gun(-1) });
  ...
  it('çok varyantlı okuma tek turda ve eksiksiz döner', async () => {
    const bilinmeyen = crypto.randomUUID();
    const map = await stocks.getAvailableMap([variantId, bilinmeyen]);
    expect(map.size).toBe(2);
    expect(map.get(bilinmeyen)?.availableQty).toBe(0);
  });

  it('partiler FEFO sırasında gelir; ürünün tarih alanları aynı turda', async () => {
    await stocks.insert({ variantId, physicalQty: 3, expiryDate: gun(300) });
    await stocks.insert({ variantId, physicalQty: 2, expiryDate: gun(100) });
...
describe('atomik ayırma (06.3)', () => {
  it('kullanılabilir yeterse ayırır ve kalanı bildirir', async () => {
    await stocks.insert({ variantId, physicalQty: 8, expiryDate: gun(200) });
  ...
  it('yetmezse HİÇ yazmaz — kısmi ayırma yok', async () => {
    await stocks.insert({ variantId, physicalQty: 2, expiryDate: gun(200) });
  ...
  it('YARIŞ: iki istek aynı anda son birimi isterse yalnız biri kazanır', async () => {
    await stocks.insert({ variantId, physicalQty: 1, expiryDate: gun(200) });
  ...
  it('YARIŞ: on eşzamanlı istek, beş birimlik stoktan yalnız beşini kapar', async () => {
    ...
    expect(sonuclar.filter((r) => r.ok)).toHaveLength(5);
    expect((await stocks.getAvailable(variantId)).availableQty).toBe(0);
  });

  it('TTL verilirse rezervasyon süreli, verilmezse süresiz (kapıda/vadeli)', async () => {
    await stocks.insert({ variantId, physicalQty: 4, expiryDate: gun(200) });
    const siparis = orderId();

    await reservations.reserve({ orderId: siparis, variantId, qty: 1, ttlMinutes: 30 });
    await reservations.reserve({ orderId: siparis, variantId, qty: 1 });

    const satirlar = await reservations.listActiveByOrder(siparis);
    expect(satirlar.filter((r) => r.expiresAt !== null)).toHaveLength(1);
    expect(satirlar.filter((r) => r.expiresAt === null)).toHaveLength(1);
  });

  it('partiye çıpalı ayırma yalnız O partinin kullanılabilirine bakar', async () => {
    const teklif = await stocks.insert({ variantId, physicalQty: 2, expiryDate: gun(30), offerPrice: 4.5 });
    await stocks.insert({ variantId, physicalQty: 20, expiryDate: gun(300) });

    // Varyant toplamı 22 olsa da çıpalı istek partinin 2'siyle sınırlıdır.
    const fazla = await reservations.reserve({ orderId: orderId(), variantId, qty: 3, stockId: teklif.id });
    expect(fazla).toMatchObject({ ok: false, available: 2 });

    const tam = await reservations.reserve({ orderId: orderId(), variantId, qty: 2, stockId: teklif.id });
...
describe('serbest bırakma ve TTL süpürme (06.4)', () => {
  it('sipariş kapanınca rezervasyonları düşer', async () => {
    await stocks.insert({ variantId, physicalQty: 6, expiryDate: gun(200) });
    const siparis = orderId();
    await reservations.reserve({ orderId: siparis, variantId, qty: 4 });

    await reservations.releaseByOrder(siparis);
...
describe('eşik altı varyantlar (06.11 girdisi)', () => {
  it('kullanılabilir eşiğin altına düşen varyant listeye girer, üstündeki girmez', async () => {
    ...
    const altinda = await stocks.listBelowMinStock();
    expect(altinda.find((r) => r.variantId === variantId)).toMatchObject({ availableQty: 4, minStockQty: 5 });

    await stocks.insert({ variantId, physicalQty: 10, expiryDate: gun(200) });
    const ustunde = await stocks.listBelowMinStock();
