...
/**
 * Kurye gün kapanışı ve kasa mutabakatı (11.6) — terfi 21.10 ile taşındı (kaynağı
 * `apps/web/lib/courier/day-close.test.ts`).
 *
...
describe('kapanış taslağı', () => {
  it('beklenen tahsilat YÖNTEM BAZINDA toplanır', async () => {
    ...
    const draft: DayCloseDraft = await openDayClose(db, { courierId, date: day });
...
describe('günü kapat', () => {
  it('sayılan beklenene eşitse gün MUTABIK kapanır', async () => {
    ...
    const result = await closeCourierDay(db, { courierId, date: day, countedCashCents: 3000 });
  ...
  it('fark AYNI GÜN görünür ve işareti anlamlıdır', async () => {
    ...
    const eksik = await closeCourierDay(db, { courierId, date: day, countedCashCents: 4500, note: 'müşteri bozuk para veremedi' });
  ...
  it('fazla para da fark sayılır', async () => {
    ...
    const fazla = await closeCourierDay(db, { courierId, date: day, countedCashCents: 2500 });
  ...
  it('sonuçlanmamış durak kapanışı ENGELLEMEZ, sayılır', async () => {
    ...
    const result = await closeCourierDay(db, { courierId, date: day, countedCashCents: 2000 });
  ...
  it('kapanmış gün İKİNCİ kez kapatılamaz — kayıt ezilmez', async () => {
    await deliverAndCollect(2, 'cash');
    const first = await closeCourierDay(db, { courierId, date: day, countedCashCents: 2000 });

    const second = await closeCourierDay(db, { courierId, date: day, countedCashCents: 99900 });
  ...
  it('kapanış sonrası hareket düzeltilse bile BEKLENEN donmuş kalır', async () => {
    ...
    await closeCourierDay(db, { courierId, date: day, countedCashCents: 3000 });
