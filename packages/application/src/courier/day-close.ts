 ...
 * `design/pages/kurye-kapanis.md` + DOMAIN §7. Terfi 21.10 (K7); web kopyası geçiş köprüsüdür.
 *
 ...
 */
export async function openDayClose(db: SupabaseClient, input: { courierId: string; date?: string }): Promise<DayCloseDraft> {
  const date = input.date ?? new Date().toISOString().slice(0, 10);

  const [stops, collection, closed] = await Promise.all([
    listCourierDay(db, { courierId: input.courierId, date }),
 ...
 */
export function closeCourierDay(
  db: SupabaseClient,
  input: {
    courierId: string;
    date: string;
  ...
  return new CourierDayCloseService(db).close({ ...input, actorId: input.courierId });
}
