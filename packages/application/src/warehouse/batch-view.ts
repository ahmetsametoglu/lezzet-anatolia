...
//
// ── TERFİ (06.13, 08.08) — `apps/web/lib/stock`ten buraya ───────────────────
// Dosya `server-only` idi ve yakın-SKT okumasının TEK adresiydi; `apps/mobile-api` onu import
// EDEMİYORDU, yani mobil D3 (yakın-SKT) ekranı açılamıyordu. Kopyalamak tüzükçe yasak (aynı eşik
// iki yerde yaşarsa bir gün ayrışır ve iki yüzey aynı parti için farklı karar gösterir) — taşındı.
//
// Paket istemci KURMAZ: `SettingsService` dışarıdan geçer. `server-only` da kalktı — taşıma
// bağımsızlığının şartı bu; Next'e bağlı ne varsa (önbellek, çerez) web köprüsünde kalır.
//
