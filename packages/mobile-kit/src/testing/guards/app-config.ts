...
/*
  ...
  MANTIK KİTTE (21.310): iki native uygulamanın `app.config.ts`i aynı tuzağa açık; her uygulama
  bekçiyi kendi kökünü vererek çağırır (`describeAppConfigGuard`).
*/
...
const PACKAGES_ROOT = path.resolve(KIT_SRC, '..', '..');
...
/** `appRoot`: native uygulamanın kökü (`app.config.ts`in klasörü). */
export function describeAppConfigGuard(appRoot: string): void {
  const appConfigSource = readFileSync(path.join(appRoot, 'app.config.ts'), 'utf8');
