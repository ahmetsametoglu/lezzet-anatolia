...
describe('endDeviceSession', () => {
  ...
  /* UYGULAMAYA ÖZGÜ TEMİZLİK (21.310): kapı ortak çekirdekte ve uygulamanın deposunu adıyla
     tanımaz; müşteri uygulamasının seçili teslimat adresi kendini kaydeder. */
  it('kayıtlı uygulama temizliklerini koşar; bırakılan kayıt koşmaz', async () => {
    const kept = jest.fn();
    const released = jest.fn();
    const releaseKept = registerSessionCleanup(kept);
    registerSessionCleanup(released)();
