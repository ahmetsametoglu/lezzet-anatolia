...
export async function exchangeOAuthCode(code: string): Promise<OAuthResult> {
  ...
  if (exchange.error === null) {
    /* ── DAVET BAĞI BURADA KURULUR (21.44 · ölçülmüş boşluk) ──────────────────
       ...
       kapattı (17.11). Kapı giriş yöntemini BİLMEZ, yalnız "oturum kuruldu"yu bilir; 21.310'dan beri
       giriş sonrası işler kaydından geçer (`sign-in-effects` — davet müşteri uygulamasının işi). */
