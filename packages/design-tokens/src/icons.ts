/*
  İKON GEOMETRİSİ — telefon yüzeylerinin ORTAK, platformdan bağımsız kısmı (kullanıcı kararı 14.09).
  ...
  İKİ SÖZLÜK, TAŞINDIKLARI GİBİ: `ICON_PATHS` native kitin sözlüğüydü (`components/ui/icon-paths.ts`),
  `CUSTOMER_ICON_PATHS` müşteri ekranlarının tamamlayıcısı (`screens/customer-kit/customer-icon.tsx`).
  Geometri ve künyeler DEĞİŞMEDEN taşındı. İkisinde aynı adla iki ayrı çizim var (`check`);
  birleştirme mobil şeridin kararıdır.
*/
...
/*
  MÜŞTERİ EKRANLARININ TAMAMLAYICISI — v3'ün müşteri ekranlarında geçen ama yukarıdaki sözlükte
  olmayan geometriler. Çizicisi native `screens/customer-kit/customer-icon.tsx` (`CustomerIcon`).
  ...
  türetmemiz olur, tasarımın söylediği şey olmazdı (yukarıdaki `circles` alanının gerekçesiyle aynı).
*/
