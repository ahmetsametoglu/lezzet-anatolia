      ...
      {showRules ? (
        <Text style={styles.rate}>
          {t.rate
            ...
            .replace('{value}', formatPrice(rules.redeem.valueCents, locale))}
        </Text>
      ) : null}
          ...
          <View key={way.key} style={styles.row} testID={`points-earn-${way.key}`}>
            ...
            {/* ── TEK SÜTUN: ÖDÜL BAŞLIĞIN ALTINDA (kullanıcı kararı 13.08) ──────────────
                ...
                *"puan kalemlerinin açıklaması, başlığı, bunların dengesi ve yerleşim şekli çok
                ...
                Puan ile para AYNI rozette ve `·` ile ayrık: kullanıcı isteği 12.08 ikisini birden
                istiyor (asıl birim puan, karşılığı para), ama alt alta iki kademe olmaları
                gerekmiyordu — tek satırda okunuyorlar. */}
            <View style={styles.body}>
              <Text style={styles.title}>{copy.title}</Text>
              <View style={styles.rewardLine}>
                <Text style={styles.rewardBadge}>
                  +{way.points} · {formatPrice(way.points * rules.centValue, locale)}
                </Text>
