...
export function ErrorState({ tone = 'neutral', icon, title, description, children }: ErrorStateProps) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 overflow-auto p-8 text-center">
      <span className={['grid h-11 w-11 place-items-center rounded-[10px]', TONE[tone]].join(' ')}>{icon}</span>
      <div className="flex flex-col items-center gap-[7px]">
        <h1 className="font-ops-display text-[21px] font-semibold text-ops-ink">{title}</h1>
        <p className="max-w-[440px] text-pretty font-ops-body text-[13.5px] leading-relaxed text-ops-body">{description}</p>
      </div>
      {children}
    </div>
  );
