...
function sectionsFor(roles: readonly UserRole[]): NavSection[] {
...
/**
 * Kullanıcı bloğunun rol satırı. Bir kişi birden çok rol taşıyabilir (depo + muhasebe) ve
 * **hepsi yazılır**: tek etiket göstermek, gezinmesinin neden o kadar geniş olduğunu gizlerdi.
 */
function roleText(roles: readonly UserRole[]): string {
...
export function AdminSidebar({ user, warehouse }: AdminSidebarProps) {
  ...
  const roleLabel = roleText(user.roles);
  const sections = sectionsFor(user.roles);
