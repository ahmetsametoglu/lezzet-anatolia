 ...
 */
export async function listCourierDay(input: {
  courierId: string;
  date?: string;
 ...
 * zorlar, kurye de en yakınına basar — yanlış veri, doğru görünümlü olur.
 */
 ...
 */
async function addressTexts(db: ReturnType<typeof serviceDb>, orders: readonly Order[]): Promise<Map<string, string>> {
...
async function variantNames(db: ReturnType<typeof serviceDb>, items: readonly OrderItem[]): Promise<Map<string, string>> {
