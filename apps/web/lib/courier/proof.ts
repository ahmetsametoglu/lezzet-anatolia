...
/**
 * **Teslim kanıtının yükleme kapısı** (11.2) — imzalı adres burada doğar.
 *
 ...
 * Tarayıcı doğrudan R2'ye yükler; sunucunun yaptığı tek şey **yetkiyi doğrulayıp kısa ömürlü bir
 * izin yazmak**. Sunucu üzerinden geçirmek fotoğrafı iki kez taşımak ve Next'in gövde sınırıyla
 * boğuşmak olurdu (emsal `lib/ticket/attachments.ts`).
 *
...
export async function requestDeliveryProofUploadUrl(input: {
  orderId: string;
  ...
  const order = await new OrderService(serviceDb()).getById(input.orderId);
...
/**
 * Siparişe yazılmış kanıdı okunur hâle getirir — **"eksik geldi" ihtilafında açılan şey budur.**
 *
