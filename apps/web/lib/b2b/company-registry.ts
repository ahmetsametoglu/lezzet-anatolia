...
export async function lookupCompanyBySiret(rawSiret: string): Promise<CompanyRegistryRecord | CompanyLookupFailure> {
  ...
  try {
    const res = await fetch(`https://recherche-entreprises.api.gouv.fr/search?q=${siret}&per_page=1`, {
      ...
      // Künye başvuru anındaki hâliyle okunur; sonraki başvuru taze cevap istesin diye önbelleklenmez.
      cache: 'no-store',
    });
    if (!res.ok) {
    ...
    await captureError(err, { source: SOURCES.webAction, context: { flow: 'b2b/lookupCompanyBySiret', siret } });
    return 'unavailable';
  }
