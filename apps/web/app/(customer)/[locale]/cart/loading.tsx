import { detectDevice } from '@/lib/device';
import { Skeleton, SkeletonBlock, SkeletonCard, SkeletonRegion, SkeletonText } from '@/components/customer/ui/skeleton';
import { PhoneCartRowsSkeleton } from './components/phone-cart-skeleton';

/**
 * Sepetin ROTA düzeyinde ilk karesi (Next `loading.tsx`).
 *
 * İki ayrı bekleme var ve ikisi de doldurulmalı: sunucu render'ı (bu dosya) ve ondan sonra sepetin
 * istemcide okunması. Yalnız ikincisini doldurmak, ilk beklemede bomboş bir sayfa bırakıyordu —
 * müşteri önce boşluğa, sonra iskelete, sonra içeriğe bakıyordu.
 *
 * Cihaz forku BURADA DA geçerli (ADR Sapma 3): ölçü sunucudan gelen ipucuyla seçilir. Telefonda çizim sepetin kendi
 * bekleme karesiyle AYNI (`cart.mobile`: başlık satırının yeri + `PhoneCartRowsSkeleton`) — iki kare arasında
 * yerleşim kaymaz (talep 11.09: sepet iskeletinin titremesi).
 */
export default async function CartLoading() {
  if ((await detectDevice()) === 'mobile') {
    return (
      <div className="flex flex-col pt-2.5">
        <div className="flex items-center gap-2 px-4 pt-1.5">
          <Skeleton className="size-10 flex-none !rounded-full" />
          <Skeleton className="h-6 w-24" />
        </div>
        <div className="px-4.5 pt-4.5">
          <PhoneCartRowsSkeleton />
        </div>
      </div>
    );
  }

  return (
    <SkeletonRegion>
      <div className="grid grid-cols-[1.6fr_1fr] items-start gap-10 px-12 pt-9 pb-12">
        <div className="flex flex-col gap-3.5">
          <Skeleton className="h-9 w-52" />
          {[0, 1, 2].map((i) => (
            <SkeletonCard key={i} className="!flex-row items-center gap-4">
              <SkeletonBlock className="size-20 flex-none" />
              <div className="flex flex-1 flex-col gap-2.5">
                <Skeleton className="h-3.5 w-3/5" />
                <Skeleton className="h-2.5 w-2/5" />
              </div>
              <Skeleton className="h-4 w-20" />
            </SkeletonCard>
          ))}
        </div>
        <SkeletonCard>
          <Skeleton className="h-5 w-32" />
          <SkeletonText lines={3} />
          <Skeleton className="mt-2 h-11 w-full rounded-pill" />
        </SkeletonCard>
      </div>
    </SkeletonRegion>
  );
}
