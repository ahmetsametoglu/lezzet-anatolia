...
export function LoginDesktop({ t, errors, subtitle, locale, stage, error, notice, isSending, emailInvalid, emailRef, emailField, onSubmit, onBack, onGoogle, onWhatsApp, onVerify, onResend }: LoginViewProps) {
  return (
    <main className="flex min-h-screen bg-cream text-[#3a3f35]">
      ...
      <div className="relative w-[42%] max-w-[560px] flex-none" style={{ background: HERO_BG }}>
        ...
        <div className="absolute inset-x-[34px] bottom-[34px] flex flex-col gap-2.5">
          {stage.kind === 'email' ? (
            <>
              <span className="font-serif text-[27px] font-medium leading-[1.32] text-[#f5f1e6]">“{t.testimonialQuote}”</span>
              <span className="font-sans text-[13px] font-semibold tracking-[.04em] text-[#dfe3cf]">{t.testimonialAuthor}</span>
            </>
          ) : (
            <span className="font-serif text-2xl font-medium leading-[1.35] text-[#f5f1e6]">{t.heroCodeText}</span>
          )}
        </div>
      </div>
