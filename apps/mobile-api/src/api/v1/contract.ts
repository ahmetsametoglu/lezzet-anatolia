 ...
 *
 * **Katalog şemaları BURADA DEĞİL ARTIK** (21.6): `CatalogCategorySchema` · `CatalogProductSchema` ·
 * `CatalogPageSchema` · `CatalogProductDetailSchema` `@lezzet/types`'ın `catalog-api.schema.ts`'ine
 * terfi etti — Expo ekranı cevabı AYNI şemayla parse ediyor, yani sözleşmenin tek kaynağı olması
 * lafta değil derlemede geçerli. Bu dosyada yalnız tek yüzeyin konuştuğu `MeSchema` kaldı.
 */
