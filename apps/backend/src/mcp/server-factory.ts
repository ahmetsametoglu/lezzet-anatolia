    ...
    inputSchema: {
      ...
      properties: {
        ...
        variants: {
          type: 'array',
          description:
            'Sizes that ALREADY EXIST on this product: [{ "variantId": "…", "netWeightG": 200, "label": { "tr": "200 g" }, "barcode": { "code": "8691234567890", "kind": "unit" } }]. variantId comes from product_detail or catalog_lookup — this tool never creates a size and never deletes one, it only fills in what a size is missing (label, netWeightG, piecesCount, portionKind, and the barcode printed on the package: kind "unit" for the consumer pack, "case" for an outer box with qtyPerCode packs per scan). A barcode\'s check digit is verified before the proposal is queued and a code already bound elsewhere is refused; read the digits off the package rather than guessing, and leave it out when you cannot read them all. A size whose netWeightG is null shows the customer no quantity at all, which the law requires before purchase, so it is worth filling. packedWeightG and packedLengthMm/packedWidthMm/packedHeightMm are the SHIPPING box: never estimated from a photo, only passed on when someone measured them.',
        },
