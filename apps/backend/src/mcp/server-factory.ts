...
export const TOOLS = [
  ...
  },
  {
    name: 'product_detail',
    description:
      'Read ONE product as it stands today — per language. Call this BEFORE propose_product_draft: that tool OVERWRITES and there is no version history, so writing blind can erase someone\'s work. For name, description, ingredients and storage you get, per locale (tr/fr/de), whether the field is filled and a short preview — enough to decide "may I write here, or would I be deleting something". Allergens come back as the list itself (a closed set, not text): null means never declared, an empty list means declared allergen-free; nutrition comes back as a yes/no. declarationGaps repeats what the engine sees missing, so you do not need a second catalog_health call; publishGaps lists what still blocks putting the product on sale, each field with the languages it lacks. Each size comes back with its label, sku, netQuantity + netUnit ("g" or "ml"; null = no quantity recorded, which blocks the sale), piecesCount and the barcodes bound to it, so you can see what is still missing on the size itself. Accepts a productId or part of a name; if several products match it returns the matches instead of guessing one.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        variants: {
          type: 'array',
          description:
            'Sizes: [{ "label": { "tr": "500 g" }, "netQuantity": 500, "netUnit": "g", "piecesCount": 12, "portionKind": "slice", "barcode": { "code": "8691234567890", "kind": "unit" } }]. At least one — a product with no size cannot be sold. barcode is the code PRINTED ON THE PACKAGE: kind "unit" for the consumer pack, "case" for an outer box, where qtyPerCode says how many packs one scan counts. Its check digit is verified before the proposal is queued, and a code already bound elsewhere is refused — so give the digits exactly as printed and leave it out when the photo is not sharp enough to read every digit. label is the TEXT the customer reads; netQuantity with netUnit ("g" for solids, "ml" for liquids — a bottle says 500 ml, not 500 g) and piecesCount are the NUMBERS behind it, and the unit price (per kg or per litre) is computed from them. A size with no quantity CANNOT go on sale. portionKind says WHAT is being counted: "item" = separate pieces (4 simit in a bag), "slice" = slices of one body (12-slice cheesecake). These are all PRINTED ON THE PACKAGE — fill them in; leave one out only when the package does not state it. SEPARATELY, and only if you actually know them: packedWeightG (gross weight in grams, product WITH its packaging) and packedLengthMm/packedWidthMm/packedHeightMm (outer box, millimetres). THESE ARE NOT PRINTED ON THE PACKAGE — they are weighed and measured. NEVER estimate them from a photo: a guessed number goes straight into the shipping tariff and the carrier corrects it on the invoice. Leave them out unless someone told you the measured values.',
        },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        variants: {
          type: 'array',
          description:
            'The product\'s sizes: [{ "variantId": "…", "netQuantity": 200, "netUnit": "g", "label": { "tr": "200 g" }, "barcode": { "code": "8691234567890", "kind": "unit" } }]. WITH variantId the row COMPLETES that existing size; WITHOUT it the row ADDS A NEW SIZE to the product — a new size needs label and netQuantity (a size with no quantity cannot be sold) and lands at the end of the list, leaving the existing sizes where they are. variantId comes from product_detail or catalog_lookup; an id that is not this product\'s is refused, and no row ever deletes a size. What a row carries is the same either way: label, netQuantity + netUnit — "ml" for liquids, piecesCount, portionKind, and the barcode printed on the package: kind "unit" for the consumer pack, "case" for an outer box with qtyPerCode packs per scan. A barcode\'s check digit is verified before the proposal is queued and a code already bound elsewhere is refused; read the digits off the package rather than guessing, and leave it out when you cannot read them all. A size whose netQuantity is null shows the customer no quantity at all — the law requires it before purchase and the product cannot be put on sale without it, so filling it is the single most useful thing here. packedWeightG and packedLengthMm/packedWidthMm/packedHeightMm are the SHIPPING box: never estimated from a photo, only passed on when someone measured them.',
        },
