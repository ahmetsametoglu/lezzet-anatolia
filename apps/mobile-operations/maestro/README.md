# Maestro — operasyon uygulamasının uçtan uca testleri

Bu klasör **operasyon native uygulamasının** akış testlerini tutar (21.310'dan beri; müşteri uygulamasının
henüz akışı yok). Repodaki öteki e2e (`/e2e`, Playwright)
...
```
pnpm mobile-operations:e2e              # tüm akışlar
pnpm mobile-operations:e2e -- depo.yaml # tek akış
```
...
```
- openLink: lezzetoperasyonu://expo-development-client/?url=http%3A%2F%2Flocalhost%3A8081
```

Şema `app.config.ts`ten gelir (`scheme: 'lezzetoperasyonu'`); değişirse buradaki bağlantı da
...
| Hepsi | `login-dev-hepsi` | Dört bölüm (sekme çubuğunun dolu hâli) |
