...
/*
  ...
  BEŞ HÂL. "Erişemiyor" ile "ÖĞRENEMEDİK" ayrı tutuluyor (CLAUDE §1 — ölçülemeyen değer sıfır
  değildir) ve "erişemiyor" da ikiye ayrılıyor (21.310 — operasyon ayrı uygulama):
  · `401` KESİN bir cevaptır — oturum yok. Karar hazır: giriş ekranı. Tek uygulama iki yüzeyi
    taşırken oturumsuz kişi müşteri kabuğuna dönüyordu (02-mimari §4); bu uygulamada dönülecek
    bir müşteri yüzeyi yok.
  · Oturum var, personel bölümü yok — o da KESİN: "bu hesabın operasyon yetkisi yok" + çıkış.
  · Ağ yok · 500 · `profile_not_found` (404) · bozuk gövde → rol bilgisi OKUNAMADI. Bu hâlde
    "yetkin yok" demek, wifi'si düşen kuryeyi kapı dışında bırakmak olurdu; ekran hatayı söyler
    ve tekrar denemeyi teklif eder.
...
type OperationsAccess =
   ...
   * depocuyu kapı dışında bırakmak, `error` dalının bütün gerekçesine ters olurdu.
   */
  | {
      status: 'granted';
...
export function useOperationsAccess(): OperationsAccess {
  ...
  const load = useCallback(async () => {
    ...
    /* DEPO KAPSAMI — `/me`den SONRA, ve yalnız kapı geçildiyse (30.08).
       ...
       ve seçici. Bir ad okunamadı diye depocuyu kapı dışında bırakmak, `error` dalının bütün
  ...
  /* OTURUM DEĞİŞİNCE KAPI YENİDEN SORULUR (21.97b — cihazda ölçüldü 22.08).
     ...
     Dinleyici müşteri tarafındaki desenin aynısı (`use-me.hook`); oturumu düşen kişiyi `signed_out`
     dalı giriş ekranına yönlendiriyor, yani karar zinciri değişmedi — yalnız tetiği doğdu. */
  useEffect(() => {
