...
| **Harita** | **`leaflet`** + Google Map Tiles API | Üç harita (checkout teslim noktası, operasyon bölge ve rota haritası) karoyu tek yerden alır (`lib/map/leaflet-base.ts`): 2D `roadmap` raster karo; oturum jetonunu sunucu alır ve bütün ziyaretçilere paylaştırır (`lib/map/tiles-session.ts`, tarayıcı anahtarı `GOOGLE_MAPS_BROWSER_API_KEY`). Google'ın şartı: sol altta Google Maps logosu, sağ altta görünen alanın telif satırı. CSP'de `tile.googleapis.com` hem `img-src` hem `connect-src`. Vektör karo (MapLibre) Web Worker + WebGL ister; raster karo `<img>` olarak yüklenir. Noktalar bizim: dışarı giden yalnız karo koordinatı. Native müşteri uygulamasında **`react-native-maps`**: iki platformda Google sağlayıcı, stil web'le ortak (`@lezzet/helper` `MAP_STYLE`), anahtarlar (`GOOGLE_MAPS_ANDROID_API_KEY` · `GOOGLE_MAPS_IOS_API_KEY`) eklentiyle derlemeye yazılır; kütüphane yüklenirken yerel modülü zorunlu istediği için nokta seçicisi onu açılınca yükler. |
...
| **Native uygulama** | **Expo SDK** + **`expo-router`** + **`expo-dev-client`** | Native uygulama `apps/mobile-customer` (21.2). CNG: native klasörler üretilir, git'e girmez; **Expo Go test hattında YASAK** — dev-client (`docs/uygulama/01 §10`). Çekirdek Expo modülleri: `expo-constants` · `expo-linking` · `expo-splash-screen` · `expo-status-bar` · `expo-system-ui` · `expo-secure-store` (oturum token'ları Keychain/Keystore'da — 21.4) · `expo-localization` (cihaz dili → `@lezzet/i18n` `LOCALES` eşlemesi; yedek `DEFAULT_LOCALE` — 21.7) · `expo-blur` (cream-glass zeminler, Token Kararı 17; Android bulanıklığı `BEKLEYEN(21.7)`) · `expo-font` (Lora/Karla statik yükleme, Token Kararı 24) · `expo-haptics` (titreşim; ekranlar doğrudan çağırmaz — niyet sözlüğü `lib/haptics` tek kapısından geçer, uygulama içi kapatma anahtarı YOK: sistem ayarı yönetir — 21.66) · `expo-camera` (Modül 23 · 22.08: YALNIZ kod okutma — vizör `components/scan/ScanSheet` tek kapısında yaşar, ekranlar kendi kamera kodunu yazmaz; fotoğraf/video çekimi yok. İzin metni 10.09'dan beri iki işi söylüyor — kod okutma + talep fotoğrafı (21.309; fotoğrafı `expo-image-picker` sistem kamerasıyla çekiyor, bu paket değil), tek cümle `app.config.ts` → `CAMERA_PERMISSION`. Ayrı tarama kütüphanesi BİLİNÇLE yok: CameraView'ın kendi barkod okuması EAN/GTIN/QR'ı kapsıyor) · `expo-modules-core` (22.08: zaten Expo'nun geçişli çekirdeği; doğrudan bağımlılığa alınmasının TEK sebebi `requireOptionalNativeModule` — kameranın native tarafı derlemede yokken FIRLATMADAN yoklamak, `components/scan/camera-availability` tek kapısında; cihazda ölçüldü: çıplak `require` try/catch içinde bile Metro redbox'ına dönüşüyor) · `expo-brother-printer-sdk` (23.5 · 22.08: Brother QL etiket yazıcısı — sistem yazdırma diyaloğu OLMADAN basım, karar §1.8; SDK'ya açılan tek kapı `lib/print/brother.ts`, native yoklama `lib/print/printer-availability` — kamera dikişinin aynı deseni. RN 0.86/New Arch bağlanması iğne deneyinin ölçtüğü şey; tutmazsa `apps/mobile-customer/modules/brother-print/` local modülü) · `expo-asset` (23.5: paketlenmiş görseli cihaz dosyasına indirip `file://` adresi almak — Brother SDK yalnız yerel dosya basıyor; Expo'nun geçişli bağımlılığıydı, pnpm izolasyonu doğrudan beyan istiyor) · `expo-file-system` (23.7: sunucunun ürettiği etiket PNG'sini cihaz dosyasına yazmak — indirme `fetch`+`File.write` ile `lib/print/label-file` tek kapısında; expo-asset'in geçişli bağımlılığıydı, aynı izolasyon gerekçesi) · `expo-audio` (21.288 · 07.09: sosyal sohbetin sesli mesajı UYGULAMA İÇİNDE çalınıyor — kullanıcı kararı *"Uygulama dışına çıkışlar olmamalı"*. `expo-av` DEĞİL: SDK 54'ten beri yerini bu paket aldı, yeni kurulumda onu seçmek bir gün gelecek taşımayı bugünden yapmak. SDK'ya açılan tek kapı `components/ui/audio-player` — ekranlar `useAudioPlayer` çağırmaz. YALNIZ ÇALMA: kayıt yok, izin/config plugin gerekmiyor (SDK 57 dokümanı). `setAudioModeAsync({playsInSilentMode:true})` şart — yoksa telefon sessizdeyken çubuk ilerler ve hiçbir şey duyulmaz. Paketin jest mock'u YOK; `src/testing/expo-audio.mock.ts` yazıldı, yoksa sesli mesaj çizen her ekran testi `import` satırında düşüyor) · `expo-image-picker` (21.309 · 10.09: talep AÇILIŞINDA fotoğraf — galeri + sistem kamerası; kullanıcı kararı *"sadece ilk başta yükleme olsun"*, yazışmada ek yok. SDK'ya açılan tek kapı `screens/support/use-ticket-photos.hook`; yükleme `lib/api/tickets` → `uploadTicketPhoto` [`expo/fetch` + `expo-file-system` `File` ile imzalı adrese `PUT` — dosya sunucudan geçmez]. Yerel modül; config plugin'i iOS izin metinlerini yazar [kamera cümlesi `expo-camera` ile ORTAK, dile göre `apps/mobile-customer/locales`], mikrofon izni kapalı. Paketin jest mock'u YOK; `src/testing/expo-image-picker.mock.ts`) · `expo-notifications` (14.14/21.13 · 26.08: cihaz push'u — izin + jeton + dokunuş; sürüm `~57.0.9` Expo'nun kendi eşleme dosyasından. SDK'ya açılan tek kapı `lib/push` — kayıt `register-device` [kanal → izin → jeton → uç, Android 13 sırası], dokunuş `use-push-navigation` [adres, uygulama içi listeyle AYNI sözlükten]. Expo Go Android'de uzak bildirim KAPALI [SDK 53+] ve `projectId`siz jeton alınamaz — iki hâl de künyeli sessizlik: uygulama içi zil aynı satırları taşıyor) · `expo-clipboard` (kupon kodunu panoya yazmak; tek kullanıcısı `screens/account/coupon-row`, başarısız yazım sessiz kalır çünkü kod ekranda okunur durur) |
...
| **Mobil yüzen sayfa (bottom sheet)** | **`@gorhom/bottom-sheet`** (+ `@gorhom/portal`) | Kullanıcı kararı 01.09: *"ihtiyaç olan kilit noktalarda bir kütüphane"* — yasak olan GÖRÜNÜŞ DAYATAN TAM KİTLER (`docs/uygulama/01-teknoloji-secimi.md §11`), odaklı kütüphane değil. Kendi yazdığımız çekmece iki kez elden geçmesine rağmen açılışı bir ÖLÇÜME bağlıydı ve ölçüm gelmediğinde panel açılmıyordu; kütüphane açılışı ölçümden bağımsız kuruyor (`enableDynamicSizing`) ve klavye davranışını da kendi taşıyor. Tek sarmalayıcı `components/ui/bottom-sheet.tsx` — 42 çağıranın hiçbiri kütüphaneyi doğrudan görmüyor; kök `BottomSheetModalProvider` `app/_layout.tsx`te. RN `Modal` KULLANILMIYOR: portal ile Modal aynı ekranda üst üste binince dokunuşlar kayboluyor |
...
- Native operasyon: `apps/mobile-operations/src/lib/operations/money.ts` (para, çekirdekten) ·
  `apps/mobile-operations/src/lib/operations/stamp.ts` (tarih-saat damgası)
- Native depo: `apps/mobile-operations/src/screens/warehouse/warehouse-format.ts` (SKT tarihi)
- Native müşteri: `apps/mobile-customer/src/screens/orders/order-format.ts` — dosyanın kendi yazdığı
...
- **Veri erişimi — çift kat savunma:** tüm okuma/yazma sunucu tarafında service-role + `lib/guard.ts` rol kapılarından geçer; RLS (satır seviyesi güvenlik) **ikinci savunma hattı** olarak temel tablolara yazılır (müşteri kendi satırı, kurye kendi teslimatı). Publishable (anon) anahtar yalnız auth ve broadcast içindir: `anon`/`authenticated` rollerinin `public` şemasında tablo, görünüm ve fonksiyon yetkisi yoktur (0001 varsayılan yetki kaldırma), görünümler `security_invoker` ile tanımlanır.
