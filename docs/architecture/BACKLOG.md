...
- **Görsel boyutlandırma — YAYIN ÖNCESİ kurgulanır** *(kullanıcı isteği 27.08)*: bugün her çerçeveye 2000 px'lik kaynak dosya gidiyor (yükleme yolunda yeniden boyutlandırma yok, `sharp` kurulu değil); native vitrinin 34–48 dp'lik daireleri için israf alan olarak ~190 kat. **Ön koşulu kendi alan adıdır** — `r2.dev`'de dönüşüm çalışmıyor (ölçüldü). Ölçekleme CDN'e devredilir, **kırpma kararı bizde kalır**: odak + zoom `trim` kesirlerine çevrilir (`gravity` değil — zoom'un karşılığı yok), formül CSS'in kadrajını birebir verdiği için dönüşüm kapalıyken CSS yolu aynı kareyi göstermeye devam eder. Üç eksik: kaynak boyutu veride yok · çerçeve oranları merkezî değil · genişlik merdiveni yok. → `build/05-katalog.md (05.37)`
