...
- **`net_quantity` + `net_unit`** — ambalajdaki net miktar ve BİRİMİ (katı `g`, sıvı `ml`) — etiket beyanı ve birim fiyat gösterimi (g → €/kg, ml → €/L). Gramla sınırlı tek kolon sıvıyı yazamıyordu ve "500 ml" etiketi hesaplanabilir bir sayı değildi. **Satıştaki boyda ZORUNLU** (tetikleyici `product_publish_requires_net_quantity`): miktar müşteriye satın almadan önce gösterilir
...
- **`packed_weight_g` · `packed_length_mm` · `packed_width_mm` · `packed_height_mm`** — **ambalajlı ürün ölçüsü** (07.12): ürün + KENDİ ambalajı. Kargo tarifesinin girdisi.
  - **`net_quantity` ile KARIŞTIRILMAZ ve adı bu yüzden sıfatlı.** Net miktar INCO beyanıdır ve birim fiyat gösterimini besler — *içindeki gıdanın* miktarı. Bu ise *taşınan şeyin* ağırlığı: 810 g'lık bir kek kutusu ambalajıyla 1,1 kg olabilir ve taşıyıcıya söylenecek sayı ikincisidir. Çıplak `weight_g` adı ilk tasarımda yazılmıştı; `net_weight_g` ile yan yana durunca hangisi olduğu okunmuyordu
