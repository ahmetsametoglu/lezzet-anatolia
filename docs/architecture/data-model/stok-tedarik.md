...
- **`storage_area_id`** — depo İÇİ alan (`StorageArea`, `restrict`) — hangi depo olduğu `warehouse_id`'de. **Serbest metindi, `19.29`da tanımlı kayda bağlandı**: `Dolap 1` ≠ `Dolap-1` gruplamayı bölüyordu ve "hangi alan boş/dolu" sorusu sorulamıyordu. `null` meşru — rafı bilinmeden de mal kabul edilir.
  **Anlamı "SON GÖRÜLDÜĞÜ YER"dir, adet dağılımı değil (kullanıcı kararı 03.09, `21.234`).** Parti tek alanda kayıtlıdır ve **depo içi taşıma kaydı YOKTUR** — elli paketin onunu soğuk odadan dondurucuya götürmek için kayıt açtırmak, tek depocunun sahasını prosedüre çevirirdi. Kontrolün gerçek hedefi (fire, son tarih, geri çağırma) partiye bağlıdır, alan-adet dağılımına değil. Adres seçim anında öğrenilir: depocu sayım/düşüm seçicisinde "hangi dolabın önündeyim" der, orada okuttuğu ya da seçtiği parti oraya yazılır (`markBatchSeen`, tek yazıcı; hareket defterine satır düşmez). Bedeli bilinçli: sistem "dondurucu boşalıyor" diyemez. Çok vardiyalı büyük depoda alan–adet tablosu (parti × alan × adet) gerekir; bugünkü kolon o kapıyı kapatmaz.
