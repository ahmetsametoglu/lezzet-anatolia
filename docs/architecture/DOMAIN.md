...
- **Her hareket izah edilebilir** (kullanıcı kararı 13.09 · ikinci karar, muhasebeci karşılaştırması): ya bir işe bağlıdır (sipariş, mal kabul, tedarikçi, transfer), ya bir **belgeye** bağlıdır (fatura, fiş, bordro, sözleşme, dekont — tutarıyla: bir ödeme birden çok belgeyi, bir belge birden çok ödemeyi kapatabilir), ya bir **türü** vardır (sözlükten TEK tür: kira, maaş, banka masrafı; isteğe bağlı hesap planı kodu). Hiçbiri yoksa "izah edilmemiş" kuyruğundadır; kayıt engellenmez, kuyruk bir iş listesidir. **Cari** "kime ödendi / kimden geldi"dir (URSSAF, muhasebeci, ev sahibi — eşleşme kelimeleri ve varsayılan türüyle), **etiket** serbest işarettir; ikisi de tek başına izah değildir. Ortağın kaydı ortak cari HESABIDIR; ortak etiketle ayrılmaz.
...
- **Stripe üç kayıttır** (kullanıcı kararı 13.09 · 12.14): tahsilat Stripe hesabına **brüt** girer; komisyon ödeme başına `stripe-ucreti` türünde gider olarak havuzdan çıkar ve siparişin `payment_fee` alanına yazılır (kârlılık onu görür); payout Stripe→banka **transferidir**, künyesinde içeriği (hangi tahsilatlar, iadeler, ücretler). Böylece defterdeki Stripe bakiyesi gerçek bakiyeyle aynıdır ve banka ekstresindeki payout satırı transferin öteki yakası olarak bağlanır.
- **Bankaya elle de yazılır, ekstre gelince birleşir** (kullanıcı kararı 13.09): "kira ödendi" o gün elle girilir; ekstre aynı parayı getirince ekstre satırı elle yazılanın bağlarını devralır ve elle yazılan silinir (izi künyede). Transferin ekstredeki yakası ise var olan uca bağlanır, ayna susar — para hiçbir hâlde iki kez sayılmaz.
...
- **Çapa İLK SİPARİŞTE kurulur, ve eşik "verildi"dir — "teslim edildi" değil (ölçüldü 26.08).** Metin bir tur *"tamamlanınca"* diyordu; uygulamada bir gün geç kaldığı görüldü: `completed` teslimden SONRA damgalanıyor ve müşteri o günden sonra bize bir daha yazmayabilir — kod hiç gitmez. Oysa kaybedecek şey (geçmiş, puan) siparişin verildiği an doğuyor ve konuşma tam o sırada canlı. Taslak ve iptal sayılmaz.
- **Kodun gönderim ANI da bir karardır: GELEN mesaj.** Serbest metin ancak 24 saatlik pencere açıkken ücretsiz gider (§11) ve sipariş anında o pencere kapalı olabilir. Gelen mesaj pencereyi tanımı gereği açar; koşul her mesajda yeniden ölçüldüğü için zamanlayıcıya, kuyruğa ve "başarısızları tekrar dene" defterine de gerek kalmaz.
- **Otomatik verilen kod GÖNDERİLEMEZSE geri alınır.** Operatör yolunda düşen gönderim sorun değil (kod insana döner, o iletir); otomatik yolda iletecek kimse yok. Satırda kalsaydı ortaya *müşterinin bilmediği bir sır* çıkardı ve dönüşünde ona cevaplayamayacağı bir soru sorulurdu — çapasızlıktan beter.
...
- **Çapayı MÜŞTERİ kurar, operatör kod üretmez (kullanıcı kararı 15.09).** Sohbet panelinden gönderilen şey bir **hesap bağlantısıdır**: müşteri bağlantıyı açar ve e-postasıyla girer (kod kendi posta kutusuna gelir) — giriş hesabı olan müşteri çapalıdır. Kanıt yine iki katlıdır: bağlantı sohbetin öteki ucuna gitti, giriş posta kutusundan geçti. Operatörün e-postaya kod gönderen ve sohbete 6 haneli kod yazan düğmeleri kalktı (15.40); yukarıdaki "kod e-postaya, cevap WhatsApp'tan" mekanizması dönüşteki soru için durur, 6 haneli kod hesabını hiç bağlamayan müşteriye otomatik verilir. İlk sipariş önerisi de bu bağlantıyı önerir.
...
- **Doğrulanamayan dönüş zarifçe düşer:** suçlama yok, sipariş engellenmiyor; yalnız geçmiş/puan/kişiye özel fiyat kapalı kalır.
- **BAĞI KOPARAN ŞEY ZAMAN AŞIMI DEĞİL, OLUMLU BİR OLAYDIR (kullanıcı kararı 26.08).** Bir tur "cevap gelmezse şu kadar gün sonra emekliye ayrılır" düşünüldü ve **geri çekildi**: o, bu bölümün kendi uyarısına düşen bir kapıydı — sessizliğe bakarak, hiçbir şey olmadan, yılda bir sipariş veren sadık müşteriyi ikiye bölerdi.
  Doğrusu şu: numara, **biri çıkıp "bu hat bende" dediğinde ve bunu kanıtladığında** el değiştirir. Kanıt iki katlıdır — kişi hesabını açmıştır (posta kutusuna gelen kodla giriş) ve hattı **şu an** elinde tutmaktadır (bağlama jetonunu o numaradan göndermiştir). Eski bağ ise hattın geçmişteki bir anına dayanır; hatlar devredilir, taze zilyetlik eskisini geçer.
  **Devir BİRLEŞME değildir:** eski kaydın siparişleri, puanları ve geçmişi yerinde kalır, taşınan yalnız KANALDIR. Emekli satır da silinmez — "bu numara bir zamanlar kimdeydi" sorusu sonradan cevaplanabilmeli.
  **Bedeli bilinerek kabul edildi:** aile telefonunda hattı sonradan bağlayan kişi kanalı devralır. Geri alınabilir (öteki kendi hattından yeniden bağlar) ve hiçbir veri kaybolmaz.
- **Hiç kaydolmayan yeni sahip, sistemin kapatamadığı tek senaryodur — ve o İNSANA düşer.** Sadece WhatsApp'tan yazan biri için olumlu bir olay hiç doğmaz: soru cevapsız kalır, kapılar kapalı kalır, ama siparişleri eski kayda yazılmaya devam eder. Buraya da zaman aşımı KONMAZ; bunun yerine hâl **operatöre görünür** kılınır (sohbet panelinde: soru kaç gündür cevapsız, o gün bugündür kaç sipariş geldi). Kararı insan verir (04.7 birleştirme/ayırma). Sipariş sayısı burada aciliyet ölçüsüdür: sıfırsa muhtemelen kimse dönmedi, artıyorsa birinin siparişleri başkasının kaydına yazılıyordur.
...
- **Puan kuralları:** puanlar **süreyle yanmaz** (süresiz birikir); yalnız **B2C (son kullanıcı)** kazanır/kullanır — B2B'nin zaten özel fiyatı var. İstismara karşı: aynı ürüne yorum/swipe **bir kez** puan verir + günlük tavan. **Redemption:** müşteri kendi isteyince çevirir (otomatik değil); bir basışta bakiye tek kupona çevrilir ama kupon en fazla `points_redeem_max` puan olur (varsayılan 2000 = 20 €), fazlası bakiyede kalır ve bir sonraki basışta çevrilir — tek kullanımlık kupon sepetle kırpılıp artanı yanmasın diye. **Yorum:** doğrulanmış alışveriş yorumu hafif moderasyonla yayınlanır.
...
- **Dış değerlendirme köprüsü:** alım-sonrası ankette memnuniyeti yüksek çıkan müşteri, akışın sonunda **halka açık bir değerlendirme sayfasına** tek-tık yönlendirilir (`FeedbackRequest` akışına bir link — yeni mekanizma değil).
  ...
  - **Varsayılan Google İşletme Profili.** Perakende **mağaza** yok ve iş yereldir: teslimat Strasbourg bölgesine rota ve bölgelerle yapılır, o yüzden Google'ın **"hizmet bölgesi" (SAB)** kaydı — adres gizli, teslimat bölgeleri beyan edilir — tam bu duruma göredir. *(Depo kapısında ve kuryenin aracından yerinde satış YAPILIR — §17 "Araç da bir depodur" — ama bu bir vitrin değildir: müşteri çekmez, ziyaret saati yoktur, haritaya adres olarak beyan edilmez. SAB kaydı bu yüzden geçerli kalır.)* Oradaki yorum haritada/yerel sonuçlarda görünür; **yeni müşteri bulduran** taraf odur. (`SCOPE §38`, `BACKLOG §221`.)
...
- **Varsayılan depo kavramı YOKTUR.** Belirsizlik varsayılanla çözülmez: sipariş deposunun kaynağı
  ya adresin posta kodudur (uzaktan sipariş: web/WhatsApp/elle) ya işlemi yapan personelin **o anki**
  deposudur (yerinde satış: depo kapısı ya da kuryenin aracı). **Admin yerinde satış yapmaz** —
  satan kişi, malın yanında duran personeldir. Yüzey sonucu (26.08): yerinde satış ekranı **native
  uygulamanın** kurye/depo bölümündedir; operasyon web'inde elle sipariş girişi yalnız *telefonla
  gelen* siparişin yazılmasıdır (`09.8`).
...
### Araç da bir depodur (kullanıcı kararı 26.08)

Kurye satılan maldan fazlasını yükleyip gittiği yerde isteyene satabilir — sahada olan bir şey. Bu
yüzden **araç bir depo TÜRÜDÜR** (`warehouse.kind` = `facility` | `vehicle`), ayrı bir kavram değil:
yükleme ve akşam dönüşü birer transfer, araçtaki mal gerçek parti. Son kullanma, FEFO, geri çağırma
izi ve soğuk zincir kendiliğinden çalışır.

- **Araç bölgeye BAĞLANAMAZ.** "Posta kodu → bölge → depo" zincirinin sonu bir adres olmak zorunda:
  rota oradan çıkar, mal kabul oraya yapılır. Zincir bir araca çözülseydi müşteri siparişi hareket
  hâlindeki bir yere yazılırdı. Kural veritabanında (tetikleyici).
- **Araç kargo deposu OLAMAZ** — taşıyıcı bir adrese gelir (kısıt).
- **Araç depo-üstü toplama girmez.** Katalog için "bizde var" bir SÖZDÜR ve araçtaki mal siteden
  alınamaz; tedarik önerisi için de o mal zaten tesisten çıkmıştır. Depo bazlı okuma aracı **aynen
  gösterir**: kurye arabasında ne olduğunu görmek zorunda.
- **Araçta iki tür mal yan yana durur ve evleri AYRIDIR.** Sipariş için yüklenen mal hâlâ tesisin
