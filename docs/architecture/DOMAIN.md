...
- **Banka import (AI):** AI ajanı banka dosyasından sütun şablonunu çıkarır (`BankImportProfile`), satırlar hesabın para hareketleri olarak girer, sonra karşılığına bağlanır (öneri + elle onay). **Her banka satırının bir karşılığı vardır** (kullanıcı kararı 13.09 · 12.13): sipariş tahsilatı, müşteri iadesi, açık belge (fatura), mal kabul (tedarikçi borcu), transferin öteki yakası (kasadan yatırma, Stripe payout'u), başka hesaba transfer, o hesaba ekstreden önce elle yazılmış hareket ya da ad koyma (gider/sermaye, etiketle). Hiçbir satır "atla"ya mecbur değildir.
- **Stripe üç kayıttır** (kullanıcı kararı 13.09 · 12.14): tahsilat Stripe hesabına **brüt** girer; komisyon ödeme başına `stripe-ucreti` gideri olarak havuzdan çıkar ve siparişin `payment_fee` alanına yazılır (kârlılık onu görür); payout Stripe→banka **transferidir**, künyesinde içeriği (hangi tahsilatlar, iadeler, ücretler). Böylece defterdeki Stripe bakiyesi gerçek bakiyeyle aynıdır ve banka ekstresindeki payout satırı transferin öteki yakası olarak bağlanır.
