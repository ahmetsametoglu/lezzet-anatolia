...
- [x] (22.42) **Tedarikçi ve cari NOKTA ATIŞI bulunur — liste yok, parça ad yok; gider önerisinde tür sözlükten, cari adla; para tipinden tedarikçi kalktı** *(kullanıcı kararı 14.09: «Yapay zekâ tüm tedarikçileri ben çekeyim, veya birkaç karakterle isimde arama yapayım — bunlar doğru değil. Tedarikçinin telefon numarası veya vergi numarası faturanın üzerinde olur zaten, veya tam adı. Nokta atışı bir arama yapması gerekir. Bu her araç için geçerli: kritik bilgileri nokta atışı talep edilirse vermek lazım.»)* · `touches: packages/domain-core/src/money/pinpoint.ts, packages/types/src/entities/assistant-proposal.schema.ts, apps/backend/src/mcp/{server-factory,tools-propose,tools-reference,proposal.test,mcp.test}.ts, packages/application/src/assistant/apply.ts, apps/web/app/(operations)/operations/assistant/{bodies/money-movement-body.tsx,assistant-preview.tsx,cards/money-card.tsx}, apps/web/components/operation/ui/payload-labels.ts, scripts/seed/assistant.ts, docs/architecture/AI_ADMIN_ASSISTANT.md`
  - *Bitti:* hiçbir araç tedarikçi ya da cari listelemiyor; mal kabul ve tedarik siparişi tedarikçiyi faturadaki vergi numarası · telefon · tam adla buluyor ve bulamayınca aday listesi dönmüyor; gider önerisi türü sözlükle doğruluyor, cariyi tam adla çözüyor; kuyruk formu cari ve tür dolu açılıyor, künyede "Kime" ve "Tür" satırı var
  - **Durum (14.09):** motor `domain-core/money/pinpoint.ts` — `pinpointSupplier` (vergi no boşluksuz büyük harf · telefon E.164 · ad sözlük slug'ı; verilen anahtarların birleşimi, iki kayda gidiyorsa "birden çok"), `pinpointCounterparty` (tam ad ya da eşleşme kelimesi), `matchNature` (slug ya da okunur ad, yöne uygun — uygulayıcı, kuyruk formu ve MCP'deki üç kopyanın yerine); 11 birim testi · `reference_data`: tedarikçi satırı kalktı, `natures` (slug · ad · yön) geldi · `propose_stock_intake`: `supplierVatNumber` · `supplierPhone` · `supplierName` (tam ad); `propose_purchase_order.supplierName` tam ad · `propose_money_movement`: `category` → `nature` (sözlükte yoksa RET, boş bırakılabilir), **`supplierName` KALKTI** — tedarikçi bu tipte hiçbir kutuya düşmüyordu (kuyruk formunun girdisinde tedarikçi yok, bağ sessizce kayboluyordu; mal bedeli mal kabule bağlı `purchase` satırıdır ve banka eşleştirmesinden yazılır, 12.13), `counterpartyName` tam ad ya da kelimeyle `counterpartyId`ye çözülür, bulunmazsa öneri yine kurulur ve cevap `counterpartyNote` taşır · sistem talimatına "tedarikçi ve cari listelenmez, parça adla aranmaz" cümlesi · payload: `nature` + `counterpartyId` (`.default(null)` — kuyruktaki eski dilekçe türsüz ve carisiz açılır), `supplierId` yok · uygulayıcı ve `movementValuesFrom` motoru okuyor; kuyruk künyesi "Kime" ve "Tür" satırlarını karşılaştırmalı çiziyor · `mcp.test.ts`: `reference_data` tedarikçi/cari taşımaz, tür satırları üç alan · `proposal.test.ts` alan denkliği kayıtları · seed: gider önerisi muhasebeci faturası (tür `muhasebe-ucreti`, cari adla, kimlik bilerek çözülmemiş).
  - **Sınır aynı kaldı, iki yere bilinçle dokunulmadı:** brifingin tedarik önerisi (`suppliersWithShortfall`) tedarikçi ADINI hâlâ söylüyor — o bir liste değil motorun hesapladığı eksik grubudur ve `propose_purchase_order` o adı tam eşitlikle alır (§3.10); hesap adları `reference_data`da duruyor — hesap kritik kayıt değil, hareket önerisinin zorunlu girdisi.
- [x] (22.43) **Mal kabul önerisinde kalem tedarikçinin adıyla çözülür; eşlemesi olmayan kalem asistanın katalog bulgusuyla "eşleme önerisi" olur ve onayda kaydedilir** *(kullanıcı kararı 14.09 — 06.16'nın asistan ayağı)* · `touches: packages/types/src/entities/assistant-proposal.schema.ts, apps/backend/src/mcp/{server-factory,tools-propose}.ts, apps/web/lib/warehouse/intake-actions.ts, apps/web/app/(operations)/operations/assistant/{assistant-body.tsx,bodies/stock-intake-body.tsx}`
  - *Bitti:* faturadaki kalem adı olduğu gibi gönderilince tedarikçinin eşlemesinden varyant bulunuyor; eşleme yoksa asistan katalogdan bulduğu varyantı adla birlikte gönderiyor, kuyrukta satır "tedarikçide: …" ile görünüyor, girişi onaylayınca eşleme yazılıyor ve sonraki fatura tam eşleşiyor
  - **Durum (14.09):** `propose_stock_intake` kalemleri `supplierItemName` · `supplierItemCode` · `variantId` (yalnız adsız ya da eşlemesiz kalemde) alıyor; tedarikçi kalemlerden ÖNCE çözülüyor, `matchSupplierItem` tedarikçinin eşlemelerinde arıyor: bulundu → varyant eşlemeden (model başka variantId verdiyse RET — eşleme doğruysa kimlik gönderilmez, yanlışsa yönetici düzeltir), birden çok → RET, yok + variantId → `mappingProposed`, yok + kimliksiz → "eşleme yok, catalog_lookup ile bulup adla birlikte gönderin" · payload satırı `supplierItemKey` · `supplierItemName` · `mappingProposed` (`.default`, eski dilekçe) · cevap `mappedFromSupplier` ve `mappingNote` · kuyruk: satır başlığı "ürün · tedarikçide: ad", künyede "Eşleme önerisi: N kalem"; onayda `receiveIntakeFromProposalAction` yalnız adet girilmiş satırların eşlemesini `setMapping` ile yazar, yazılamayan eşleme girişi geri almaz ve cevapta söylenir · SKT kuralı değişmedi: faturada SKT yok, kalem etiketten okunur; ~~faturadan "beklenen teslimat" ayrı iş~~ (22.44'te geldi).
- [x] (22.44) **Faturadan belge, tedarikçi ve faturadan sipariş önerileri; mal kabul önerisinde fatura belge olarak doğar; `money_overview` bekleyen işi sayar** *(kullanıcı soruları 14.09: "belge önermek demek belgeyi yüklemek mi demek?" · "alım faturası sisteme girebiliyor mu?" · "bu konu kapandı mı? … gidip sen oradaki ilgili yerleri tamamla")* · `touches: supabase/migrations/0042_assistant_proposal.sql, packages/types/src/entities/assistant-proposal.schema.ts, packages/application/src/{assistant/kind-meta,assistant/apply,warehouse/supplier,index}.ts, apps/backend/src/mcp/{server-factory,tools-propose,tools-signals,proposal.test,mcp.test}.ts, apps/web/app/(operations)/operations/assistant/**, apps/web/lib/{warehouse/intake-actions.ts,warehouse/intake-types.ts,stock/purchase-order-actions.ts,assistant/form-options.ts}, apps/web/components/operation/ui/payload-labels.ts, scripts/seed/{assistant,coverage,assistant.test}.ts`
  - *Bitti:* asistan mal dışı faturayı belge, faturanın başlığını tedarikçi kartı, mal gelmeden kesilen faturayı gönderilmiş sipariş olarak öneriyor; mal kabul önerisinde faturanın toplamı okunduysa fatura kabule bağlı belge olarak doğuyor; dosya onay formunda bırakılıyor; `money_overview` izah bekleyen hareket, açık belge ve faturasız kabul sayılarını veriyor
  - **Durum (14.09):** iki yeni tip `money_document` · `supplier_create` (enum, şema, künye `inline`, kuyruk rengi, köprü adresi — Para'nın Belgeler sekmesi, Tedarik'in Tedarikçiler sekmesi) · araçlar `propose_money_document` (tedarikçi nokta atışı ya da cari tam ad/kelime; tür sözlükten; rejim ülkeden önerilir, KDV'yle çelişen rejim RET; aynı numara aynı tarafa ikinci kez yazılamaz) · `propose_supplier_create` (vergi no · telefon · tam adla kayıtlı tedarikçi varsa RET ve adını söyler) · `propose_stock_intake`: `vatAmountCents` · `vatRegime` · `dueOn`; fark hesabı KDV hariç tutarla (her KDV'li faturada "fark" KDV'nin kendisi çıkıyordu), cevapta belge notu · `propose_purchase_order` fatura kipi: tedarikçi faturadaki kimlikle, kalemler tedarikçinin adıyla eşlemeden (`resolveInvoiceLines` — mal kabulle ortak), birim fiyat faturadan; onayda sipariş GÖNDERİLMİŞ açılır (`sendPurchaseOrder`, tedarikçiye mesaj gitmez), fatura siparişe bağlı belge olur, eşleme önerileri yazılır · `money_overview.attention` (adsız sayılar) · kuyruk: belge gövdesi Para ekranının belge formuyla, tedarikçi gövdesi Tedarik ekranının kart formuyla; mal kabul ve sipariş gövdelerinde fatura bloğu (toplam · KDV · rejim · vade) ve dosya alanı, dosya belge yazıldıktan sonra yükleniyor; düğme metni ve karar notu dilekçeye göre (`applyLabelOf` · `appliedNoteOf`); kartlar ve künye etiketleri · eski uygulayıcı faturalı siparişi reddediyor (belgesiz açılmasın); yeni iki tipin uygulayıcısı ekranın kapısından — belge `createMoneyDocument`, tedarikçi `packages/application/src/warehouse/supplier.ts` (alanların kuruluşu ve mükerrer yoklaması Tedarik ekranının eylemiyle ORTAK; eylemdeki kopya kalktı) · seed iki yeni öneri, kapsam cümlesi tip sayısından bağımsız · testler: alan denkliği kayıtları, kuyruk içi tip listesi, tutar ve etki türetmeleri, iki uygulayıcı (tedarikçi kartı açılır ve aynı vergi no reddedilir; belge ters yüklemeyle yazılır, KDV'li ters yükleme reddedilir), MCP kapsam sayısı 13 · araçlar yerel veritabanında doğrudan çağrıyla denendi: 10 durumun 10'u beklendiği gibi (kayıtlı tedarikçi, ters yüklemede KDV, bilinmeyen tür, iki karşı taraf, bilinmeyen tedarikçi, kalemsiz fatura ve bozuk vade reddedildi; tedarikçi, belge ve faturadan sipariş önerileri kuruldu, sipariş toplam kontrolünde fark 0). **Görsel denetim yapılamadı:** 3001'deki çekim başka şeridin eşzamanlı derlemesiyle geçersiz kaldı (14.09 gecesi).
  - **Dosya MCP'den geçmez:** araçların girdisi yalnız metin; asistan faturayı okur, dosyanın kendisini sisteme taşıyamaz. Onay formu dosyayı kabul ediyor. Faturaların e-postadan doğrudan bir belge kutusuna düşmesi ayrı bir iş olarak konuşuldu, açılmadı.
