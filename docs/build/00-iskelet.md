...
- [~] (00.9) **Playwright — önce GÖZ, sonra duman** (kullanıcı kararı 03.08; denetim etüdü) · `touches: playwright.config.ts, scripts/ui-shot.mjs, e2e/**, package.json`
  ...
  - **Senaryolar ÇAPRAZ yazılır (kullanıcı kararı 03.08):** müşteri yüzeyinin dumanını operasyon şeridi, operasyonunkini müşteri şeridi yazar — kaynak kod değil tasarım+DOMAIN; **bakım ekran sahibinde** (ekranı değiştirip testi kızartan düzeltir).
  - Kalan: dev server açıkken canlı doğrulama + Kademe 2 senaryolarının çapraz yazımı (şeritler) + müşteri OTP kod-yakalama kapısı.
