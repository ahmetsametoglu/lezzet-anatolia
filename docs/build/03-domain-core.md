...
- [~] (03.12) **İkiz kuralları teke indir ya da karşılaştır** — denetim 27.08'de motorun 11 kuralının üretimde hiç sorulmadığını ölçtü (üç bağımsız ölçüm aynı kümede buluştu). Dört ayak; ikisi kapandı, ikisi KULLANICI KARARI bekliyor:
  ...
  - ~~**Kaldırılabilir ikiz — uzaklık** (`distanceKm`/`nearestOf` ↔ `routes-suggest.ts`)~~ — 27.08'de birleştirildi, ayak kapandı. Sayfa klasöründeki kopya söküldü; `routes.desktop.tsx` artık hem uzaklığı hem *"en yakınını seç"* kararını motora soruyor (ikincisi elle `Math.min` ile yazılmıştı). Birleştirme aynı zamanda düzeltme: kopyada motorun iki koruması yoktu — koordinat eksikse `null` dönüşü ve `asin` girdisinin kırpılması; artık *"hiç çıpa yok"* ile *"çıpaların koordinatı yok"* aynı cevabı veriyor ve **ikisi de sıfır sayılmıyor** (`CLAUDE §1` "ölçülemeyen değer SIFIR değildir"). Kopyanın iki testi silindi, kapsam kaybı yok: motorun `distance.test.ts`i ikisini de içeriyor, üstüne kopyada hiç olmayan `null` dalını ve simetriyi de sınıyor.
  - **Kaldırılabilir sanılan ikiz — depo seçimi** (`warehouseOptions` ↔ `readWorkWarehouse`): birleştirmeye hazırlanırken ölçüldü ve **aynı kural olmadığı çıktı.** Motor `limited` dalında ikinci argümanını (`allWarehouseIds`) hiç kullanmıyor, kapsamı olduğu gibi döndürüyor; çağıran ise `ctx.warehouses` kullanıyor — `activeOnly: true` ve kapsamla süzülü. Fark gerçek: iki depoya atanmış personelin depolarından biri KAPATILMIŞSA motor *"seçim gerekiyor"* der, doğru cevap *"tek depo var, sorma"*dır. **Naif delegasyon çalışan bir davranışı bozardı.** `priceIn` ile aynı desen: kopya sanılan şey başka bir soru — motorunki *"kapsam neye izin veriyor"*, çağırandaki *"şu an nereye yazacağım"*. Ayrıca `readWorkWarehouse`un **hiç testi yok**; testsiz davranış, doğruluğu kanıtlanamadan değiştirilemez. Önerilen sıra: (1) `readWorkWarehouse`a test yazıp bugünkü davranışı sabitle, (2) `warehouseOptions`u `limited` dalında da kesişim alacak şekilde düzelt, (3) sonra delege et. Bu bir tasarım iyileştirmesi, hata düzeltmesi DEĞİL — fonksiyon üretimde hiç çağrılmadığı için bugün zarar verdiği kanıtlanamaz.
