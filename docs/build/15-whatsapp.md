...
- [x] (15.1) `Conversation` + `Message` servisleri ve elle konuşma/mesaj kaydı (admin, gelen DM'i işler) `touches: supabase/migrations/0039_conversation.sql, packages/types/src/entities/conversation.schema.ts, packages/database/src/services/conversation.service.ts, packages/domain-core/src/messaging/**, apps/web/lib/whatsapp/**`
  ...
  - **Pencere hesabında iki para kusuru bulundu ve kapatıldı (08.08 · kullanıcı maliyet önceliğini söyleyince ölçüldü).** İkisi de sessizdi — hata vermez, yalnız fatura büyütür:
    ...
    **(a2) Pencere yazılıyor ama OKUNMUYORDU** — ölçülüp kullanılmayan bir sayı. `serviceWindowState` eklendi (domain-core): "serbest metin mi, ücretli şablon mı" sorusunun tek kapısı. `recordOutboundMessage` artık ona bakıyor ve pencere açıkken kaydedilen şablonu işaretliyor — bedava olana ödenen paranın izi. Kayıt **reddedilmiyor**: adım 1'de mesaj zaten gönderilmiş oluyor, olmuş bir şeyin kaydını reddetmek defteri yalancı yapardı. Engelleyen kapı 15.11'de.
...
- [ ] (15.11) **Utility template'ler:** sipariş onayı / kargo bildirimi şablonları onaylatılır; `packages/notify` WhatsApp API sürücüsü doldurulur; pencere içi serbest mesaj / pencere dışı template kararı `Conversation`'dan
  ...
  - **Maliyet raporu BİLEREK yazılmadı** (ekranı doğduğu gün tek turda eklenir) — ama sayımın **kategori kırılımı şart**: fiyat kategoriye bağlı (pazarlama her hâlde ücretli; utility açık pencere içinde ücretsiz olabiliyor). Düz `kind='template'` sayımı üç farklı fiyatı tek toplama atardı. Kategori kolonu bu yüzden defterle birlikte doğuyor → 15.1 durum notu.
