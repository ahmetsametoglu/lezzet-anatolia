...
- [ ] (14.10) **Bekleyen müşteriye SONUÇ bildirimi — iki olay, aynı boşluk** *(04.08'de iki ayrı şeritten aynı sınıf bulundu)*: `b2b_application_result` (onay/ret) + `zone_available` (beklenen bölge açıldı) · `touches: packages/notify/src/types.ts, packages/email/src/templates/**, packages/database/src/services/zone-notice.service.ts, apps/backend/src/jobs/**`
  ...
  - **Kanal bugün yalnız E-POSTA.** WhatsApp `15.x` inmeden gidemez ve ekran "WhatsApp da gitti" dememeli; `packages/notify` yeteneğe kendi bakıyor (`supports`), yani kanal açıldığında iş değişmeden ikincisi devreye girer.
