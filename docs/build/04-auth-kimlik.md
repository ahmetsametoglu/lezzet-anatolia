...
- [x] (04.12) **HIZLI GİRİŞ KAPISI — production derlemesinde de gerçek oturum** (kullanıcı isteği 15.08).
  ...
  **ÖLÇÜLMEYEN:** production sunucusunda (3001) aynı tur. O an web derlemesi PARALEL ŞERİDİN
  ...
  - **AÇIK KALAN (görev `[~]`):** motorun **KAPILARI ve TETİKLERİ yazıldı ama HİÇBİR YERDE UYGULANMIYOR.** `canOpenHistory` (geçmiş/puan/kişiye özel fiyat) ve `needsChallenge` (3 ay sessizlik · taşıyıcının `failed` beyanı) saf fonksiyonlar olarak duruyor ve testli; onları çağıran bir tüketici yok — yani *"3 ay sessizlik sonrası dönüşte geçmiş kod sorulmadan açılmıyor"* bitti-kriteri **karşılanmadı**. Bağlanacağı yerler: ajanın bağlam kurucusu (`conversationContextOf`) ve puan harcama kapısı. `customer_phone.retired_at` de hâlâ yazıcısız — emekliye ayıran yol o tetiklerle birlikte doğar. Taşıyıcı statü olayları (`failed`) 15.7'nin webhook'unda henüz numara kaydına yazılmıyor; erken tetik onsuz ölçülemez.
