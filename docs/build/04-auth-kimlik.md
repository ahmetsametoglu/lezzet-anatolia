...
- [x] (04.12) **HIZLI GİRİŞ KAPISI — production derlemesinde de gerçek oturum** (kullanıcı isteği 15.08).
  ...
  **ÖLÇÜLMEYEN:** production sunucusunda (3001) aynı tur. O an web derlemesi PARALEL ŞERİDİN
  ...
  - **AÇIK KALAN (görev `[~]`) — TEK MADDE: `customer_phone.retired_at` hâlâ YAZICISIZ.** DOMAIN §10 *"doğrulanamayan dönüş zarifçe düşer: eski kimlik emekliye ayrılır, yeni müşteri kaydı açılır, sipariş oraya yazılır"* diyor. Bugün kapılar kapanıyor ve soru soruluyor, ama **cevap hiç gelmezse hiçbir şey olmuyor** — numara eski kimliğe bağlı kalıyor ve yeni sahibi kendi kaydını hiç açamıyor. Emeklilik bir zaman aşımı kararıdır (kaç mesaj/kaç gün cevapsızlık) ve o eşik kullanıcıya sorulacak; kod tarafı hazır (`retired_at` + kısmi unique indeks + `findActive` süzgeci).
    **İkinci, küçük madde:** puan kapısı İKİ gövdeye birden yazılmak zorunda kaldı çünkü çevirmenin iki kopyası var (`BEKLEYEN(04.10)`, ayrıntı ve yön 17.5'in durum notunda). Bugün ikisi de aynı kuralı uyguluyor; tekleşene kadar birlikte hareket etmeleri gerekiyor.
