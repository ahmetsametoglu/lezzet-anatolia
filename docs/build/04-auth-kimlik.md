...
- [x] (04.12) **HIZLI GİRİŞ KAPISI — production derlemesinde de gerçek oturum** (kullanıcı isteği 15.08).
  ...
  **ÖLÇÜLMEYEN:** production sunucusunda (3001) aynı tur. O an web derlemesi PARALEL ŞERİDİN
  ...
  - **AÇIK KALAN (bu görev hâlâ `[ ]`):** çapa akışının kendisi — e-posta bağlama (kod e-postaya, cevap WhatsApp'tan) ve 6 haneli güvenlik kodu; ardından kapılar (geçmiş/puan/kişiye özel fiyat) ve emeklilik tetikleri (3 ay sessizlik · taşıyıcının `failed` beyanı). `customer_phone.retired_at` bugün yazıcısız duruyor ve o tetiklerle dolacak.
