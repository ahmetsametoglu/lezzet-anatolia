...
- [~] (11.11) **Adres DOĞRULANABİLİRLİĞİ — "bu kapı var mı" sorusu hiç sorulmuyor:** kaba eşleşme tespit edilir, müşteriye düzeltme teklif edilir, düzeltilmezse sevkiyat ve kurye uyarılır
  ...
  - **Durum (13.09) — ALMANYA YARISI BAĞLANDI: Google Address Validation (`@lezzet/address-google`).** `geocoder()` ülkeyi SORGUDAN okur — FR BAN, DE Google (anahtar `GOOGLE_MAPS_API_KEY`, yalnız `delivery/google-maps.ts` okur; yokken `unsupported_country`, kapı susar — davranış aynı). Aynı iki soru, TEK çağrı: gövde sokak satırı + kod + şehir taşır; Google kodu DEĞİŞTİRDİYSE (`replaced`) kapı istenen kodda yoktur → `no_match`, `elsewhere` AYNI cevabın düzeltilmiş adresini tek aday taşır (ağa ikinci kez çıkılmaz) → `addressVerdict` "yanlış kod" teklifini kurar. BAN'ın kısıtsız ikinci araması Google'da KULLANILMAZ — bağlamsız soru rastgele kapı seçiyor (canlı ölçüm aşağıda). Skor servisin bayraklarından türer (kapı + tam 0,95 · doğrulanmamış bileşen 0,85 · sokak 0,6) ki 0,8 teklif eşiği iki kaynakta aynı anlama gelsin. `geo_source` enum'una `google` (`0011` yerinde düzenlendi — `db:refresh` gerekir, kullanıcı izni 13.09). **30 gün kuralı (Google politikası):** tarama işi her turda önce yaşlanmış Google noktalarını düşürür (`expireGoogleGeo` · `AddressService.listStaleGeo`) ve DE satırlarını taramada SORMAZ (`geocoderScanAllowed`) — ücretli ve 30 gün ömürlü kaynak sipariş anına saklı, koordinatın gerçek ömrü sipariş↔teslimat penceresi (02.09). 401/403 `denied`, öteki 4xx `rejected` adıyla gelir; ikisi de geçici DEĞİL ve `traceGoogleFailure` (`delivery/google-maps.ts`) `captureError` ile iz bırakır — kapı yine susar. Adaptör sözleşmesi testle çivili (ağsız, gövde ölçülür); paket istemcisi ayrıca testli — **ama paket hiçbir vitest projesine yazılmamıştı, testleri HİÇ koşmuyordu** (13.09'da birim projesine eklendi; iki dosya 30/30). Müşteri yüzeyinde DE düzeltme teklifinin yanında Google Maps atfı — adres formu turuyla birlikte (`08.41`/`19.7`). **Canlı ölçüm (13.09, gerçek anahtar):** Places öneri + detay ilk denemede çalıştı; Address Validation önce anahtarın API kısıtından 403 aldı (kullanıcı listeye ekledi), sonra üç kusur çıktı ve düzeltildi — (1) gövdedeki üst düzey `languageCode` isteği 400 ile düşürüyordu ve paket bunu `unavailable` diye gizliyordu; (2) `elsewhere`in yalnız sokak sorusu başka şehri seçti ("Hauptstraße 1" → 84544 Aschau am Inn; müşteri 77694 Kehl'de) ve bu, müşteriye düzeltme teklifi olarak gidecekti; (3) `denied` ve 400 hiçbir yere yazılmıyordu. Düzeltme sonrası aynı adres: 77694 → kapı, 0,95; 02000 → `no_match` + tek aday 77694 Kehl (0,85) → "yanlış kod" teklifi.
