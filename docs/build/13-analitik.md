...
- [x] (13.1) **Olay toplama:** sunucu-tarafı `AnalyticsEvent`; çerezsiz oturum anahtarı; **kişisel kimlik YOK** · `touches: supabase/migrations/0035_analytics.sql, packages/types/src/entities/analytics.schema.ts, packages/database/src/services/analytics.service.ts, apps/web/lib/analytics/**, apps/backend/src/jobs/analytics-rollup.ts`
  ...
  - **Durum (04.08) — ŞEMA + KAPI + İŞ TAMAMLANDI; kalan yalnız ATICILAR (08.9, müşteri şeridi).**
    ...
    - **ÜLKE VE DİL BESLENDİ, SATIR KAPANDI (07.08).** İki boyut da kolon olarak duruyordu ama her satırda `null`'dı.
      - **`country` IP'den değil ÇÖZÜLMÜŞ YERDEN geliyor — ve künye bu yüzden `ANALYTICS §2`'de düzeltildi.** Eski cümle *"IP saklanmaz, yalnız `country` türetilir"* diyordu; hiç uygulanmamıştı ve **uygulanamazdı**: kolonun tipi `CountryEnum = ['FR','DE']`, yani iki değerli bir teslimat kümesi. IP'den türeyen bir ülke tam ISO listesi ister — Belçika'dan gelen ziyaretçi o kolona yazılamazdı bile. Belgelenen anlam seçilen tiple baştan çelişiyordu; tip haklıydı, cümle yanlıştı. Besleyen artık müşterinin posta koduyla çözdüğü yer (`readPlaceAnswer`, istek başına önbellekli — ek sorgu yok) ve ticari eksen de bu: "hangi ülkenin bölgesine bakıyor". `null` gerçek bir kova: yer henüz çözülmedi.
      - **`language` sitenin o anki dili** (`getLocale()`), tarayıcı tahmini değil — ziyaretçinin `/fr/…` önekiyle yaptığı BEYAN. Değer kapalı kümeye karşı doğrulanıyor: doğrulamasaydık sözlük dışı bir dize enum kolonuna gider ve yazma veritabanında patlardı. Bağlam kurulamamışsa `null` dönüyor, **fırlatmıyor** — dil uğruna olayın tamamını düşürmek "ölçüm akışı kesmez" ilkesini bozardı.
...
- [x] (13.4) **Talep sinyalleri:** ürün-ilgi (çok bakılıp az alınan), site içi arama + **sıfır-sonuç** (talep/çeşit sinyali), aday ürün talep panosu (**beğeniler `ProductFeedback`'ten okunur**, analitikten değil)
  ...
  - **DİL KIRILIMI BİLİNÇLİ OLARAK ERTELENDİ (09.08 — kullanıcının sorusu, arka uç ölçümü).**
    Soru şuydu: *"aynı ürün üç ayrı URL'den açılıyor; analitikte ayrı ayrı mı yazılıyor? ve dile göre ürün tercihi görülebilecek mi?"*
    - **Birleştirme SORUN DEĞİL, çalışıyor.** Ölçüldü: defterdeki tek `product_view` kalıbı `/product/[slug]`. İki bağımsız sebepten: slug dil-bağımsız (`PATHNAMES` künyesi) ve `routePattern` dil önekini atıp `produit`/`produkt`/`urun`'u aynı iç kalıba çözüyor. Ürün kırılımı zaten yola hiç bakmıyor, `product_id` ile toplanıyor.
    - **Dil ham defterde VAR, hiçbir özette YOK.** `analytics_event.language` yazılıyor (`record.ts`) ama `analytics_daily` de `analytics_daily_product` da dil taşımıyor; ekranlar kural gereği yalnız özet okuduğu için (`ANALYTICS §5`) bugün görünmüyor. `read.ts`'te tek bir `language` geçişi yok.
    - **Ertelemenin gerekçesi ÖRNEKLEM, maliyet değil.** Boyut eklemek pahalı da değil: ürünü `analytics_daily`'ye koymama gerekçesi *sınırsız* çarpandı, dilin doğal tavanı 3. Ama 141 ürünlük katalogda sıralamanın okunabilmesi için sınırdaki üründe 20-30 görüntüleme gerekir → **dil başına birkaç bin ürün görüntülemesi**. Bugün defterde 9 olay var (yerel, hepsi `tr`); şimdi kurulan ekran boş tablo teslim eder ve gürültüye bakarak alınan karar bakmadan alınandan kötüdür.
    - **Kayıp yok, ve kararı geciktirmeyi mümkün kılan da bu:** dil ham defterde 25 ay duruyor ve `build_analytics_daily_product` idempotent — boyut sonradan eklendiğinde geçmiş günler yeniden üretilerek kapsanır. "Ölçmeye hiç başlamamak" geri alınamaz tek karardı ve o doğru verilmiş.
    - **Tetik:** bir dil, karşılaştırma penceresinde birkaç bin ürün görüntülemesine ulaştığında. O gün yapılacak iş: `analytics_daily_product` anahtarı `gün × ürün × dil` olur **ve iki tüketici dilleri toplayacak biçimde düzeltilir** — `readShowcase` (`lib/storefront/home.ts`) ile MCP sinyalleri (`apps/backend/src/mcp/tools-signals.ts`). Atlanırsa vitrin sessizce tek dilin sayısına göre seçim yapmaya başlar; hata vermez.
    - ⚠ **Bu ARAYÜZ DİLİ, milliyet değil.** Strasbourg'da Türkçe konuşan müşteri siteyi Fransızca gezebilir. Sinyal gerçektir ama *"Almanlar şunu seviyor"* diye okunamaz; okunabileceği şey *"Fransızca gezen ziyaretçi bu ürüne bakıyor ama sepete atmıyor → anlatım eksik"*.
...
- [~] (13.5) **Segmentler:** edinim kaynağı kohortu (tekrar sipariş), RFM + uyuyan müşteri (siparişten türetilir), export'lu
  - *Bitti:* "90 gündür sipariş vermeyenler" listesi türetiliyor; ~~export çalışıyor~~ **DIŞA ALMA HENÜZ YOK** (düzeltildi 15.08 — aşağıdaki Durum notu)
  - **Durum (15.08 — SATIR YANLIŞ ŞEY SÖYLÜYORDU; dışa alma tasarımı kayda indirildi).** *Bitti:* satırı *"export çalışıyor"* diyordu; ölçüm bunu çürütüyor: `lib/analytics/` içinde `export.ts` **yok**, `operations/analytics/` içinde "Dışa al" düğmesi **yok**. Görev `[~]` olduğu için satır bir yalan değil ama tamamlanmış gibi okunuyordu — `CLAUDE.md §5`'in *"tamamlanmış görev satırı vaat ettiğini teslim etmiş olmalı"* kuralı.
    **Kapı bir kez YAZILDI ve SİLİNDİ, sebebi kayıtlı** (`operasyon-segment-disa-alma-tasarimi.md`, arka uç şeridi 07.08): `lib/analytics/export.ts` yazıldı, test edildi, geri alındı — **çağıranı yoktu** ve `knip` haklı olarak kesti. Düğme operasyon klasöründe, kapı `lib/`de; ikisi ayrı turda yazılırsa biri ölü kod olur. **Bu yüzden tek turda yapılmalı:** düğme + sunucu eylemi + kapı aynı hamlede.
    **Kapının tasarımı hazır, yeniden düşünülmeyecek:** `exportSegment(segment, options) → { rows, csv, truncated }`. Dört kararı:
    1. **İzin sütunu ŞART ve ÜÇ hâlli** (`izin var` · `reddetti` · `sorulmadı`). Dosya bir pazarlama listesine dönüşecek; sütun olmasaydı operatör listeyi e-posta aracına yapıştırır ve izin vermemişlere gönderirdi — **hiçbir yerde hata vermeden GDPR ihlali**. Son iki hâl birleştirilemez: sorulmamış müşteriye izin isteyen bir e-posta gönderilebilir, reddetmiş olana gönderilemez. Kanallar da bağımsız (e-posta izni WhatsApp izni değildir).
    2. **İzinsizler SÜZÜLMEZ, GÖSTERİLİR.** Sessizce düşürmek segmentin gerçek büyüklüğünü gizlerdi: "214 uyuyan" diyen ekranın altından 96 satırlık dosya çıkardı ve fark bir arıza sanılırdı. Karar operatörün, veri bizim.
    3. **Kimlik birleştirmesi UYGULAMA katmanında** — `ANALYTICS §6` özeti kimlikten uzak tutuyor ve bu doğru; analitik servisine kimlik `join`'i eklemek defterin kimliksizlik iddiasını tek satırda çürütürdü.
    4. **Tavan var ve kesilme SESSİZ DEĞİL** (`truncated`): müşteri kümesi veriyle sınırsız büyür, eksik olduğu söylenmeyen bir liste tam sanılır — ekran "ilk N satır" demeli.
    Sütunlar: Ad · E-posta · Telefon · Sipariş · Ciro · Son sipariş · E-posta izni · WhatsApp izni.
  ...
  - **SATIR AÇIK KALIYOR — dışa alma yok, ve 07.08'de yazılıp GERİ ALINDI.** Bitti-kriteri iki şey istiyor: liste **ve export**. Liste 04.08'de çıktı. Dışa alma kapısı (`lib/analytics/export.ts`) yazıldı, test edildi, sonra **silindi** — çünkü çağıranı yoktu: indirme düğmesi operasyon şeridinin klasöründe (13.8) ve o klasöre dokunmuyorum. `knip` dışa açık ama tüketilmeyen fonksiyonu 1 ile kesti ve haklıydı. Bu oturumda üç kez adını koyduğum arıza sınıfının ("motor yazılmış, çağrılmamış") kendisiydi; bu sefer yapan bendim ve makine yakaladı.
    - **Ders:** kapının doğru zamanı, tüketicisinin doğduğu turdur. Bir tur önce yazılan kapı ölü koddur; şerit sınırı yüzünden tüketici başka birindeyse, kapı da o turda yazılmalı — önce değil.
    - **Tasarım boşa gitmedi**, `docs/talep`'e devredildi: izin sütunu ÜÇ hâlli (`izin var` · `reddetti` · `sorulmadı` — son ikisini birleştirmek izin kanıtını bozar) · izinsizler **süzülmez, gösterilir** (süzmek segmentin gerçek büyüklüğünü gizlerdi) · kimlik birleştirmesi uygulama katmanında, analitik servisinde değil (`STACK §4` — servise kimlik `join`'i defterin kimliksizlik iddiasını çürütürdü) · tavan + `truncated` bayrağı (eksik olduğu söylenmeyen liste tam sanılır).
